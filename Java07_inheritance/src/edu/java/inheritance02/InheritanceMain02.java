...
//		Person() 생성자 호출. >> 상위 클래스 먼저 생성
//		BusinessPerson() 생성자 호출. >> 하위 클래스 생성
		
		BusinessPerson p2 = new BusinessPerson("아이티윌");
//		Person() 생성자 호출.
//		BusinessPerson(company) 생성자 호출.
		
		BusinessPerson p3 = new BusinessPerson("김아무개", "아이티윌");
//		Person(name) 생성자 호출.
//		BusinessPerson(name, company) 생성자 호출.
		
		// 하위 클래스의 객체를 생성하기 위해서 생성자를 호출하면
		// 상위 클래스의 생성자가 먼저 실행되고, 그 후에 하위 클래스의 생성자가 실행됨.
		// 하위 클래스의 생성자에서 상위 클래스의 생성자를 호출하려면 super(...)를 사용.
		// 하위 클래스 생성자에서 상위 클래스의 생성자를 명시적으로 호출하지 않으면, 
		// -> 상위 클래스의 "기본 생성자(default constructor)"가 암묵적으로 호출된다.
