...
import static edu.java.interface01.DatabaseModule.DB_VERSION; // 아래에서 더 간단하게 코드 작성 가능.
...
public class InterfaceMain01 {
	...
	public static void main(String[] args) {
		...
		//-> import static 문장을 사용하면 간단히 코드 작성 가능. (DatabaseModule.DB_VERSION > DB_VERSION)
//		DatabaseModule.DB_VERSION = 2;
		//-> The final field DatabaseModule.DB_VERSION cannot be assigned
		
//		new DatabaseModule(); //>> Cannot instantiate the type DatabaseModule
		//-> 인터페이스는 객체를 생성할 수 없음.
		
		// 인터페이스를 구현하고 있는 클래스의 객체는 생성할 수 있음.
//		OracleDateabaseModule db = new OracleDateabaseModule();
		
//		MariaDBModule db = new MariaDBModule();
		
		// 다형성: SuperType var = new SuvType();
		// 인터페이스 이름도 구현 클래스들의 상위 타입으로 상용할 수 있음.
//		DatabaseModule db = new MariaDBModule();
		DatabaseModule db = new OracleDateabaseModule();
