...
public class ThreadMain02 {
	...
	public static void main(String [] args) {
		 ...
		 * 2. 구현 클래스에서 Runnable 인터페이스의 run 메서드를 재정의 - Thread가 할 일
		 * 3. Thread 생성자를 호출할 때 Runnable 구현 객체를 argument로 전달.
		 * 		Thread(Runnable runnable) 생성자 호출 -> Thread 객체 생성
		 * 4. Thread 객체의 start 메서드 호출 -> thread 실행 -> run 자동 실행
		 * 
		 */
		...
		// (3) Thread 객체 생성
		Thread th1 = new Thread(new MyRunnable("안녕"));
		
		// 익명 클래스를 사용해서 thread 객체 생성
