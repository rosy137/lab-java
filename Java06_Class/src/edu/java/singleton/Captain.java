...
public class Captain {
	...
	// (3) public static Captain(<-리턴값) 메서드
	...
	public static Captain getInstance() {
		if (instance == null) { // Captain 타입으로 생성된 객체가 없으면,
			...
			// Captain 타입 객체를 생성하고, 생성된 객체의 주소값을 instance에 저장
		}
		return instance; // Captain 타입 객체의 주소값을 리턴.
	}
	
//	인스턴스가 널(객체가 생성되지 않은 상태)이면 생성자 작동,
//	인스턴스가 널이 아니면(객체가 생성되어있으면) 생성자 스킵, 이미 만들어진 인스턴스 호출
//	그렇기 때문에 객체는 오직 한번만 생성된다.
